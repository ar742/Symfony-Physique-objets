...
Le [cinquième exemple](docs/EXEMPLE-GLOBAL-BRANCHES.md), sous `/graphes/production/branches`, propose un unique préréglage sur huit nœuds et douze branches, avec des lois différentes selon les branches. Chacune produit y=x f(x) ; les nœuds additionnent et partagent entièrement les productions. Le témoin initial atteint déjà le maximum global r=7/32=0,21875 sur les cinq partages à lois fixées. La grille explore ses configurations et le solveur établit ses propres bornes. Le panneau avancé de paramètres bornés commence à min=max aux lois courantes ; modifier ces plages change le problème. Les configurations, flux et certificats sont exportables en JSON. Les anciens préréglages 0,54432, b=0,83 et le bouton libre sont retirés de l’interface ; leurs études restent dans la documentation historique.

Son [explorateur et sa preuve analytique](docs/EXEMPLE-GLOBAL-BRANCHES.md) séparent les rendements compatibles en x12/x23 ou s1/s2, les trois nappes libres du lagrangien non linéaire exact, et le diagnostic affine d’un PL réellement résolu. Les multiplicateurs analytiques donnent L_NLP≤7/32 sur toute la boîte et le témoin atteint cette borne. Les dérivées, Hessiennes et seuils sont explicités ; les points libres hors contraintes ne sont pas présentés comme des rendements réalisables. Ce certificat particulier est indépendant des bornes calculées par le solveur.

Le sommet intérieur lisse se situe en (x12,x23)=(0,5 ; 0,125), ou (s1,s2)=(0,5 ; 0,5). Il est strict dans ces coupes, avec les fractions aval déclarées, mais non unique dans les cinq partages du réseau. Le rayon, le maillage 20/40/60 et le zoom vertical absolu servent à lire la courbure ; les profils, voisins et marqueurs conservent leurs valeurs calculées et leurs conditions de validité.
