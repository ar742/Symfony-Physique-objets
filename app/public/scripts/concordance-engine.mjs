/** Nodal concordance model: Y=max(0,X*(environment+incoming influences)). No DOM. */
...
export function validateConcordanceModel(model) {
    ...
    for(const id of IDS) {
        ...
        for(const [from,value] of Object.entries(model.epsilon[id]??{}))if(!Object.hasOwn(epsilon[id],from)&&value!==0)throw new RangeError(`L’influence ${from}→${id} n’appartient pas au graphe et doit être nulle.`);
    }
    for(const [id,row] of Object.entries(model.epsilon))if(!IDS.includes(id)&&Object.values(row??{}).some(value=>value!==0))throw new RangeError(`Aucune influence entrante n’est définie pour le nœud ${id}.`);
...
function common(options={}) {
    ...
    if(!['rectified','efficiency'].includes(domain))throw new RangeError('Domaine rectified ou efficiency requis.');
...
function evaluateScalar(model,controls,options) {
    ...
    for(const id of ORDER) {
        ...
        const rawOutput=input*coefficient,output=id==='1'?1:Math.max(0,rawOutput);
        ...
        nodes.push({id,input,coefficient,rawOutput,output,rectified:rawOutput<0});
    ...
    const objectives={arrivals:IN['8'].reduce((total,i)=>total+q[i],0),output:nodes.find(node=>node.id==='8').output};
...
function evaluateReady(model,controls,options,detailed=true) {
    ...
    for(const id of ORDER) {
        if(id!=='1') {
            ...
            const output=raw.value>0?raw:jet(0);
            if(raw.value===0&&!raw.constant)kinks.push({node:id,input:input.value,coefficient:coefficient.value,rawGradient:[...raw.gradient],reason:'Produit X·C nul : dérivées non retenues sans analyse supplémentaire du seuil.'});
        ...
        nodes.push({id,input:node.input.value,coefficient:node.coefficient.value,rawOutput:node.raw.value,output:node.output.value,rectified:node.raw.value<0});
    ...
    const arrivals=sum(IN['8'].map(index=>q[index])),output=nodeJets['8'].output,chosen=options.objective==='arrivals'?arrivals:output;
    const differentiable=kinks.length===0&&chosen.gradient.every(finite)&&chosen.hessian.flat().every(finite);
    return {controls:{...controls},nodes,flows,objectives:{arrivals:arrivals.value,output:output.value},objective:chosen.value,
...
function oneInputRange(range,e,epsilon) {
    const candidates=[range[0],range[1]];
    if(epsilon!==0){const stationary=-e/(2*epsilon);if(stationary>range[0]&&stationary<range[1])candidates.push(stationary);}
    const values=candidates.map(x=>ia(im(iv(e),iv(x)),im(iv(epsilon),im(iv(x),iv(x)))));
...
function boundReady(model,box,options) {
    ...
    for(const id of ORDER) {
        if(id!=='1') {
            ...
            nodes[id]={input,coefficient,output:nonnegative(raw)};
        ...
        OUT[id].forEach((index,position)=>{
            ...
            q[index]=nonnegative(im(nodes[id].output,nonnegative(fraction)));
        });
    }
    const arrivals=isum(IN['8'].map(i=>q[i])),selected=options.objective==='arrivals'?arrivals:nodes['8'].output;
...
export function evaluateConcordanceLagrangian(model,point,options={}) {
    ...
    IDS.forEach((id,i)=>{
        const coefficient=ready.environments[id]+IN[id].reduce((sum,index)=>sum+ready.epsilon[id][EDGES[index].from]*q[index],0),raw=X[id]*coefficient,phi=raw>0?1:0;
        ...
        if(raw===0&&multipliers[7+i]!==0)kinks.push({type:'rectification',node:id,selection:0});
    });
    IDS.forEach((id,i)=>constraints.push({id:`law-${id}`,value:Y[id]-Math.max(0,X[id]*coefficients[id]),multiplier:multipliers[7+i]}));
    ...
    for(let n=1;n<=7;n++){const id=String(n),supply=Y[id];for(const index of OUT[id])if(supply<0||q[index]<0||q[index]>supply+1e-10)shareAdmissible=false;}
    const domainAdmissible=Object.values(X).every(value=>value>=0)&&Object.values(Y).every(value=>value>=0)&&(mode.domain!=='efficiency'||Object.values(coefficients).every(value=>value>=0&&value<=1));
    return {point:[...point],objective,lagrangian,multipliers:[...multipliers],constraints,coefficients,residual,feasible:residual<=1e-10&&shareAdmissible&&domainAdmissible,shareAdmissible,domainAdmissible,
        ...
        convention:'hX=X−Σq ; hY=Y−max(0,XC) ; hshare=Σq−Y, avec Y1=1 ; L=objectif+λhX+μhY+ηhshare.'};
}
...
export function explainConcordanceLagrangian(model,state,options={}) {
    ...
    for(const id of [...ORDER].reverse().filter(id=>id!=='8')) {
        ...
        for(const index of OUT[id]) {
            const edge=EDGES[index],node=byId[edge.to],phi=node.rawOutput>0?1:0;
            if(node.rawOutput===0)selections.push({node:edge.to,selection:0,reason:'Seuil de rectification : choix déclaré, pas une dérivée certifiée.'});
    ...
    const mu=IDS.map(id=>-p[id]),lambda=IDS.map((id,i)=>mu[i]*(byId[id].rawOutput>0?1:0)*byId[id].coefficient),eta=Array.from({length:7},(_,i)=>-p[String(i+1)]),multipliers=[...lambda,...mu,...eta];
