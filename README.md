...
Le [cinquième exemple](docs/OPTIMISATION-BRANCHES.md), sous `/graphes/production/branches`, reprend le réseau de huit nœuds et douze liaisons fixé par l’auteur. Les fonctions sont portées par les branches : source 1, sommes aux nœuds, cinq partages, trois contributions au résultat r. Il compare une grille de partages et une recherche globale avec bornes lagrangiennes, puis permet de faire varier les paramètres dans des plages indépendantes. Chaque branche ouvre son analyse Exp.–TH–Exp. et sa courbe ; les configurations et certificats sont exportables en JSON. Chaque branche produit y=x f(x), où f est son coefficient de rendement ; r≤1. Les paramètres libres disposent d’une construction atteignant r=1, soit 100 % sans perte. La recherche globale encadre les productions quadratiques par morceaux.

Son [explorateur des lagrangiens et surfaces](docs/LAGRANGIEN-SURFACES.md) distingue le plan du vrai lagrangien d’un PL enregistré, les coupes de partages et la surface des alimentations x12/x23. Un voisinage resserré autour du meilleur résultat, un zoom vertical annoncé, des coupes orthogonales et un tableau de voisins rendent le maximum de frontière et sa cassure lisibles. Les lois et commandes restantes sont figées ; les marqueurs projetés sont recalculés. La courbe couplée de la première version est retirée. Ces vues conservent les contraintes et la portée des bornes globales.

Le maillage est réglable à 20, 40 ou 60 divisions par axe ; une fenêtre élargie révèle mieux la courbure du scénario initial. Un exemple complémentaire, explicitement doté d’autres lois fixes, présente un sommet intérieur lisse en (x12,x23)=(0,5 ; 0,125), de rendement 7/32=0,21875. Sa formule locale, sa Hessienne négative et sa borne globale sont expliquées ; les deux méthodes de calcul le vérifient sans recevoir cette solution analytique comme certificat. Le résultat initial 0,54432 reste distinct et accessible.
