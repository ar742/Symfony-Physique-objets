...
- Un journal de **sept corrections ou clarifications** des passages utilisés des recueils.
...
Ouvrir [le site local](http://localhost:8080/), [les fiches](http://localhost:8080/fiches/) ou [le diagnostic](http://localhost:8080/installation). Adapter le port dans les liens s'il a été changé.

La boucle et les huit fiches sont présentes dès le démarrage. Les tables d'archives sont initialement vides ; leur import est facultatif. Sur un PC où le projet modernisé fonctionne déjà, publier ce dépôt ne nécessite pas de réinstaller le site.
...
Pour activer les liens PDF, copier les éditions correspondantes sous les noms suivants :
...
Les fichiers sont servis en lecture seule. [Les empreintes et repères de pages](app/config/content/recueils.json) doivent correspondre à l'édition copiée. Sans ces fichiers, les fiches web et les sommaires restent consultables, mais les liens vers les PDF retournent « document indisponible ».
