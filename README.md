...
Le cinquième atelier, sous `/graphes/production/branches`, propose exactement deux préréglages sur huit nœuds et douze branches : le [cas de maximum 7/32=0,21875](docs/EXEMPLE-GLOBAL-BRANCHES.md), puis le [complément à douze branches actives](docs/EXEMPLE-ACTIF-BRANCHES.md), de maximum 0,447553125. Chacune produit y=x f(x) ; les nœuds additionnent et partagent entièrement les productions. Les deux départs sont déjà optimaux à leurs lois fixées. Dans le complément, les sommes reçues aux nœuds 2 à 8 ont au départ un rapport maximal de 1,47738, inférieur à 1,5 : la source est exclue, sans renormalisation. La grille explore ses configurations et le solveur établit ses propres bornes. Le panneau avancé de paramètres bornés commence à min=max aux lois courantes ; les modifier change le problème. Les anciens préréglages 0,54432, b=0,83 et le bouton libre restent retirés de l’interface.

L’explorateur sépare les rendements compatibles en x12/x23 ou s1/s2, les trois nappes libres du lagrangien non linéaire exact, et le diagnostic affine d’un PL réellement résolu. Chaque préréglage possède sa preuve analytique par multiplicateurs, avec une borne atteinte propre à ses lois. Les dérivées, Hessiennes et seuils sont explicités ; les points libres hors contraintes ne sont pas présentés comme des rendements réalisables. Les preuves particulières restent distinguées des bornes numériques indépendantes et de leurs statuts. Les configurations, flux, vues et certificats sont exportables en JSON.

Le cas 7/32 présente un sommet strict dans ses deux coupes mais des partages aval maximisants non uniques. Le complément actif a un sommet strict et unique dans les cinq fractions, toutes intérieures ; sa grille de pas 0,1 manque le partage optimal s5=0,25 et affiche donc un résultat légèrement inférieur. Le rayon, le maillage 20/40/60 et le zoom vertical absolu servent à lire la courbure ; les profils, voisins et marqueurs conservent leurs valeurs calculées et leurs conditions de validité.

Vérification du volet : `node --test app/tools/test-branches-engine.mjs app/tools/test-branches-parameter-envelope.mjs` et, dans l’application Symfony, `php tools/verify-branches.php`. Les paramètres et résultats restent locaux au navigateur ; aucun PDF commercial ni export SQL n’est publié.
