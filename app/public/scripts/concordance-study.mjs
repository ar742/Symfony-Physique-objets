...
function stop(message) {
    ...
    if(phase&&results[phase]?.partial)results[phase]={...results[phase],status:'cancelled',complete:false};
...
function apply() {
    try {
        ...
        next.epsilon=Object.fromEntries(ids.map(id=>[id,{}]));
        ...
        const result=evaluateConcordance(next,next.initialControls,options);
        if(!result.feasible)throw new Error(result.reason);
        stop();model=next;initial=result;resetResults();$('concordance-error').textContent='';
...
function load() {
    stop();model=createConcordanceScenario();showDraft();initial=evaluateConcordance(model,model.initialControls,options);
...
function compare() {
    ...
    try {
        ...
        worker=new Worker(new URL('./concordance-worker.mjs?v=20260914',import.meta.url),{type:'module'});
        ...
        worker.postMessage({model,settings});
...
function renderResults() {
    ...
    if(results.global)$('concordance-results').append(element('p','La borne globale combine la propagation d’intervalles et des bornes sur les flux traversant des coupures du réseau. Les multiplicateurs du lagrangien affichés plus bas sont un diagnostic local distinct. Un écart non fermé reste visible.','small-note'));
}
function inspect() {
    ...
    renderInspector({edge:'Nœud '+selected,variant:'1b',anchorPrefix:'concordance-noeud-'+selected,
        ...
        points:[
            ...
            {title:selected==='1'?'Répartir la source':'Appliquer la loi avec mise à zéro',tip:'Le coefficient dépend des contributions reçues, avec leurs concordances.',paragraphs:selected==='1'?['Y₁=1 ; s₁ est envoyé vers 2 et 1−s₁ vers 5.']:[`C${selected} = ${num(model.environments[selected])} + ${terms||'0'} = ${num(node.coefficient)}. Chaque terme ajouté vaut ε${selected},j × qj,${selected}.`,`Y${selected} = max(0, X${selected} × C${selected}) = ${num(node.output)}. Une sortie négative devient zéro ; une sortie positive n’est pas plafonnée.`]},
...
function renderLagrangian() {
    ...
    try {
        ...
        zone.append(element('p','Dans une portion régulière, φᵢ=1 si XᵢCᵢ>0 et φᵢ=0 si XᵢCᵢ<0. ∂L/∂Xᵢ=λᵢ−μᵢφᵢCᵢ ; ∂L/∂Yᵢ=μᵢ−ηᵢ pour i<8 ; ∂L/∂Y₈=1+μ₈.','branch-formula'));
...
function render() {
    ...
    const nodes=table(['Nœud','Exp. IN X','Coefficient brut C','Produit brut X × C','Exp. OUT Y = max(0, XC)'],'État calculé ; la source impose Y₁=1');
    ...
    $('concordance-state').replaceChildren(element('p',`${names[view]} : r = Y₈ = ${num(s.objective)} ; somme reçue en 8 = ${num(s.nodes.find(n=>String(n.id)==='8')?.input)}.`,'branch-focus-value'),nodes.wrap);
    ...
    const matrix=table(['Destination i / fournisseur j',...ids],'Matrice ε : coefficients des liaisons effectives ; zéro sur la diagonale et hors du graphe');
    ids.forEach(id=>addRow(matrix.body,[id,...ids.map(from=>num(model.epsilon[id]?.[from]??0))]));$('concordance-matrix').replaceChildren(matrix.wrap);
...
function renderSurface() {
    ...
    try {
        ...
        zone.append(element('p','dXᵢ = Σⱼ dqⱼᵢ ; dCᵢ = Σⱼ εᵢⱼ dqⱼᵢ. Si XᵢCᵢ>0, dYᵢ = Cᵢ dXᵢ + Xᵢ dCᵢ ; si XᵢCᵢ<0, dYᵢ=0. Puis dqᵢₖ = sᵢₖ dYᵢ + Yᵢ dsᵢₖ. Au seuil, les dérivées doivent être examinées séparément.','branch-formula'));
...
try {
    ...
    $('concordance-export').addEventListener('click',()=>{
        const record={version:1,model,semantics:{objective:'Exp. OUT₈ après TH₈',negativeOutputs:'replaced-with-zero',positiveOutputs:'not-capped',epsilon:'destination-then-source'},options,settings,initial,results,timesMilliseconds:times,displayedView:view,lagrangian:windowDiagnostic,surface};
    ...
    $('concordance-node').value=selected;load();restoreNode(Boolean(location.hash));
