...
/** A compatible section with the same signed/rectified law as its reference. */
export function sampleConcordanceSurface(model, state, {x='s1',y='s2',radius=.15,steps=30}={}) {
    ...
    const options={objective:'output',domain:state.domain??'rectified'};
    ...
    return {axes:{x,y},radius,steps,points,reference,profiles,ranges,
        ...
        objective:'Exp. OUT₈ après TH₈',domain:options.domain,constraints:`Lois ${options.domain==='signed'?'signées':'rectifiées'}, associations et partages recalculés à chaque point.`};
}
